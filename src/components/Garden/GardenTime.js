...
import { getLastValue } from "../../services/SoilSensorService";
...
function GardenTime({ hour, minute }) {
  ...
  const getWaterPump = async () => {
    try {
      let res = await getDeviceById(1012);
  ...
  const getSoilSensor = async () => {
    let res = await getLastValue();
    if (res && res.data && res.status === 200) {
      setHumidity(res.data.value);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="content-container">
        <span className="body1-bold" style={{ color: "var(--bold-brown)" }}>
          Độ ẩm
        </span>
      ...
      <SetWateringTime
        ...
        handleWateringDurationChange={handleWateringDurationChange}
      />
      <div
        style={{
          width: "100%",
