...
import { changeStateDevice, getAllDevices } from "../services/UserService";
import {
  getAllDevicesInRoom,
...
const DeviceRegion = () => {
  ...
  const weeklyDataExhaust = [
    [700, 73, 205],
    [100, 45, 300],
    [600, 15, 200],
  ];
  const monthlyDataExhaust = [
    [500, 40, 450],
    [30, 35, 200],
    [25, 50, 105],
  ];
  ...
  const getListDevice = async () => {
    try {
      let res = await getAllDevicesInRoom(location);
  ...
  const handleRemoveDevice = async (device) => {
    let res = await removeDeviceFromRoom(location, device.id);
    if (res && res.data) {
  ...
  const hasPumpDevice = devices.some(
    (device) => device.deviceName === "Máy bơm"
  );

  // Kiểm tra nếu có thiết bị "Máy lạnh" trong danh sách
  const hasAirConditioner = devices.some(
    (device) => device.deviceName === "Cảm biến DHT11 phòng ngủ"
  );

  // Kiểm tra nếu có thiết bị "Máy lạnh" trong danh sách
  const hasExhaust = devices.some((device) => device.deviceName === "");
          ...
          <tbody>
            {devices.map((item) => (
              <tr key={item.id}>
                ...
                <td>
                  {item.state === Number(1) ? "Hoạt động" : "Không hoạt động"}
                </td>
                <td>
                ...
                <td>
                  <a
                    href={`/devices/consumption/${item.id}`}
                ...
                <td>
                  <button onClick={() => handleRemoveDevice(item)}>
